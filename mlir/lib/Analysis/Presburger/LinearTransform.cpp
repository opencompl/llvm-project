...
static void modEntryColumnOperation(Matrix &m, unsigned row, unsigned sourceCol,
  ...
  TPInt ratio = m(row, targetCol) / m(row, sourceCol);
...
IntegerRelation LinearTransform::applyTo(const IntegerRelation &rel) const {
  ...
  for (unsigned i = 0, e = rel.getNumEqualities(); i < e; ++i) {
    ArrayRef<TPInt> eq = rel.getEquality(i);

    TPInt c = eq.back();

    SmallVector<TPInt, 8> newEq = preMultiplyWithRow(eq.drop_back());
    newEq.push_back(c);
    result.addEquality(newEq);
  }

  for (unsigned i = 0, e = rel.getNumInequalities(); i < e; ++i) {
    ArrayRef<TPInt> ineq = rel.getInequality(i);

    TPInt c = ineq.back();

    SmallVector<TPInt, 8> newIneq = preMultiplyWithRow(ineq.drop_back());
    newIneq.push_back(c);
