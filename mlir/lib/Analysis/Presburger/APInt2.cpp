//===-- APInt2.cpp - Implement APInt2 class ---------------------------------===//
//
...
void APInt2::initSlowCase(uint64_t val, bool isSigned) {
...
void APInt2::initSlowCase(const APInt2& that) {
  U.pVal = getMemory(getNumWords());
  memcpy(U.pVal, that.U.pVal, getNumWords() * APINT_WORD_SIZE);
}

void APInt2::initFromArray(ArrayRef<uint64_t> bigVal) {
...
APInt2::APInt2(unsigned numBits, ArrayRef<uint64_t> bigVal) : BitWidth(numBits) {
  initFromArray(bigVal);
}

APInt2::APInt2(unsigned numBits, unsigned numWords, const uint64_t bigVal[])
    : BitWidth(numBits) {
  initFromArray(makeArrayRef(bigVal, numWords));
}

APInt2::APInt2(unsigned numbits, StringRef Str, uint8_t radix)
    : BitWidth(numbits) {
  fromString(numbits, Str, radix);
}

void APInt2::reallocate(unsigned NewBitWidth) {
...
void APInt2::assignSlowCase(const APInt2 &RHS) {
...
/// This method 'profiles' an APInt2 for use with FoldingSet.
void APInt2::Profile(FoldingSetNodeID& ID) const {
...
/// Prefix increment operator. Increments the APInt2 by one.
APInt2& APInt2::operator++() {
  if (isSingleWord())
    ++U.VAL;
  else
...
/// Prefix decrement operator. Decrements the APInt2 by one.
APInt2& APInt2::operator--() {
  if (isSingleWord())
    --U.VAL;
  else
...
/// Adds the RHS APInt2 to this APInt2.
/// @returns this, after addition of RHS.
/// Addition assignment operator.
APInt2& APInt2::operator+=(const APInt2& RHS) {
...
APInt2& APInt2::operator+=(uint64_t RHS) {
  if (isSingleWord())
    U.VAL += RHS;
  else
...
/// Subtracts the RHS APInt2 from this APInt2
...
APInt2 APInt2::operator*(const APInt2& RHS) const {
  assert(BitWidth == RHS.BitWidth && "Bit widths must be the same");
  if (isSingleWord())
    return APInt2(BitWidth, U.VAL * RHS.U.VAL);

  APInt2 Result(getMemory(getNumWords()), getBitWidth());
  tcMultiply(Result.U.pVal, U.pVal, RHS.U.pVal, getNumWords());
  Result.clearUnusedBits();
  return Result;
}

void APInt2::andAssignSlowCase(const APInt2 &RHS) {
  WordType *dst = U.pVal, *rhs = RHS.U.pVal;
  for (size_t i = 0, e = getNumWords(); i != e; ++i)
    dst[i] &= rhs[i];
}

void APInt2::orAssignSlowCase(const APInt2 &RHS) {
  WordType *dst = U.pVal, *rhs = RHS.U.pVal;
  for (size_t i = 0, e = getNumWords(); i != e; ++i)
    dst[i] |= rhs[i];
}

void APInt2::xorAssignSlowCase(const APInt2 &RHS) {
  WordType *dst = U.pVal, *rhs = RHS.U.pVal;
  for (size_t i = 0, e = getNumWords(); i != e; ++i)
    dst[i] ^= rhs[i];
}
...
bool APInt2::equalSlowCase(const APInt2 &RHS) const {
  return std::equal(U.pVal, U.pVal + getNumWords(), RHS.U.pVal);
}

int APInt2::compare(const APInt2& RHS) const {
...
void APInt2::setBitsSlowCase(unsigned loBit, unsigned hiBit) {
...
static void tcComplement(APInt2::WordType *dst, unsigned parts) {
  for (unsigned i = 0; i < parts; i++)
    dst[i] = ~dst[i];
}

/// Toggle every bit to its opposite value.
void APInt2::flipAllBitsSlowCase() {
...
APInt2 APInt2::concatSlowCase(const APInt2 &NewLSB) const {
  unsigned NewWidth = getBitWidth() + NewLSB.getBitWidth();
  APInt2 Result = NewLSB.zextOrSelf(NewWidth);
...
void APInt2::flipBit(unsigned bitPosition) {
  assert(bitPosition < BitWidth && "Out of the bit-width range!");
  setBitVal(bitPosition, !(*this)[bitPosition]);
}

void APInt2::insertBits(const APInt2 &subBits, unsigned bitPosition) {
...
void APInt2::insertBits(uint64_t subBits, unsigned bitPosition, unsigned numBits) {
...
APInt2 APInt2::extractBits(unsigned numBits, unsigned bitPosition) const {
  assert(bitPosition < BitWidth && (numBits + bitPosition) <= BitWidth &&
         "Illegal bit extraction");

  if (isSingleWord())
    return APInt2(numBits, U.VAL >> bitPosition);
  ...
  if (loWord == hiWord)
    return APInt2(numBits, U.pVal[loWord] >> loBit);

  // Extracting bits that start on a source word boundary can be done
  // as a fast memory copy.
  if (loBit == 0)
    return APInt2(numBits, makeArrayRef(U.pVal + loWord, 1 + hiWord - loWord));

  // General case - shift + copy source words directly into place.
  APInt2 Result(numBits, 0);
...
uint64_t APInt2::extractBitsAsZExtValue(unsigned numBits,
...
unsigned APInt2::getSufficientBitsNeeded(StringRef Str, uint8_t Radix) {
...
unsigned APInt2::getBitsNeeded(StringRef str, uint8_t radix) {
  // Compute a sufficient number of bits that is always large enough but might
  // be too large.
  ...
  APInt2 tmp(sufficient, StringRef(p, slen), radix);
...
hash_code llvm::hash_value(const APInt2 &Arg) {
  if (Arg.isSingleWord())
...
unsigned DenseMapInfo<APInt2, void>::getHashValue(const APInt2 &Key) {
  return static_cast<unsigned>(hash_value(Key));
}

bool APInt2::isSplat(unsigned SplatSizeInBits) const {
...
/// This function returns the high "numBits" bits of this APInt2.
APInt2 APInt2::getHiBits(unsigned numBits) const {
  return this->lshr(BitWidth - numBits);
}

/// This function returns the low "numBits" bits of this APInt2.
APInt2 APInt2::getLoBits(unsigned numBits) const {
  APInt2 Result(getLowBitsSet(BitWidth, numBits));
  Result &= *this;
  return Result;
}

/// Return a value containing V broadcasted over NewLen bits.
APInt2 APInt2::getSplat(unsigned NewLen, const APInt2 &V) {
  assert(NewLen >= V.getBitWidth() && "Can't splat to smaller bit width!");

  APInt2 Val = V.zextOrSelf(NewLen);
...
unsigned APInt2::countLeadingZerosSlowCase() const {
  unsigned Count = 0;
...
unsigned APInt2::countLeadingOnesSlowCase() const {
...
unsigned APInt2::countTrailingZerosSlowCase() const {
  unsigned Count = 0;
  unsigned i = 0;
...
unsigned APInt2::countTrailingOnesSlowCase() const {
  unsigned Count = 0;
  unsigned i = 0;
...
unsigned APInt2::countPopulationSlowCase() const {
  unsigned Count = 0;
...
bool APInt2::intersectsSlowCase(const APInt2 &RHS) const {
...
bool APInt2::isSubsetOfSlowCase(const APInt2 &RHS) const {
  for (unsigned i = 0, e = getNumWords(); i != e; ++i)
    if ((U.pVal[i] & ~RHS.U.pVal[i]) != 0)
      return false;
...
APInt2 APInt2::byteSwap() const {
  assert(BitWidth >= 16 && BitWidth % 8 == 0 && "Cannot byteswap!");
  if (BitWidth == 16)
    return APInt2(BitWidth, ByteSwap_16(uint16_t(U.VAL)));
  if (BitWidth == 32)
    return APInt2(BitWidth, ByteSwap_32(unsigned(U.VAL)));
  if (BitWidth <= 64) {
    uint64_t Tmp1 = ByteSwap_64(U.VAL);
    Tmp1 >>= (64 - BitWidth);
    return APInt2(BitWidth, Tmp1);
  }

  APInt2 Result(getNumWords() * APINT_BITS_PER_WORD, 0);
...
APInt2 APInt2::reverseBits() const {
  switch (BitWidth) {
  case 64:
    return APInt2(BitWidth, llvm::reverseBits<uint64_t>(U.VAL));
  case 32:
    return APInt2(BitWidth, llvm::reverseBits<uint32_t>(U.VAL));
  case 16:
    return APInt2(BitWidth, llvm::reverseBits<uint16_t>(U.VAL));
  case 8:
    return APInt2(BitWidth, llvm::reverseBits<uint8_t>(U.VAL));
  case 0:
    return *this;
  default:
    break;
...
APInt2 llvm::APInt2Ops::RoundDoubleToAPInt2(double Double, unsigned width) {
  ...
  if (exp < 52)
    return isNeg ? -APInt2(width, mantissa >> (52 - exp)) :
                    APInt2(width, mantissa >> (52 - exp));

  // If the client didn't provide enough bits for us to shift the mantissa into
  // then the result is undefined, just return 0
  if (width <= exp - 52)
    return APInt2(width, 0);

  // Otherwise, we have to shift the mantissa bits up to the right location
  APInt2 Tmp(width, mantissa);
  Tmp <<= (unsigned)exp - 52;
  return isNeg ? -Tmp : Tmp;
}

/// This function converts this APInt2 to a double.
...
///  --------------------------------------
double APInt2::roundToDouble(bool isSigned) const {
...
APInt2 APInt2::trunc(unsigned width) const {
  assert(width < BitWidth && "Invalid APInt2 Truncate request");

  if (width <= APINT_BITS_PER_WORD)
    return APInt2(width, getRawData()[0]);

  APInt2 Result(getMemory(getNumWords(width)), width);
...
APInt2 APInt2::truncUSat(unsigned width) const {
  assert(width < BitWidth && "Invalid APInt2 Truncate request");
  ...
  return APInt2::getMaxValue(width);
}

// Truncate to new width with signed saturation.
APInt2 APInt2::truncSSat(unsigned width) const {
  assert(width < BitWidth && "Invalid APInt2 Truncate request");
  ...
  return isNegative() ? APInt2::getSignedMinValue(width)
                      : APInt2::getSignedMaxValue(width);
}

// Sign extend to a new width.
APInt2 APInt2::sext(unsigned Width) const {
  assert(Width > BitWidth && "Invalid APInt2 SignExtend request");

  if (Width <= APINT_BITS_PER_WORD)
    return APInt2(Width, SignExtend64(U.VAL, BitWidth));
...
APInt2 APInt2::zext(unsigned width) const {
  assert(width > BitWidth && "Invalid APInt2 ZeroExtend request");

  if (width <= APINT_BITS_PER_WORD)
    return APInt2(width, U.VAL);
...
APInt2 APInt2::zextOrTrunc(unsigned width) const {
  if (BitWidth < width)
    return zext(width);
  if (BitWidth > width)
    return trunc(width);
...
APInt2 APInt2::sextOrTrunc(unsigned width) const {
  if (BitWidth < width)
    return sext(width);
  if (BitWidth > width)
    return trunc(width);
...
APInt2 APInt2::truncOrSelf(unsigned width) const {
  if (BitWidth > width)
    return trunc(width);
  return *this;
}

APInt2 APInt2::zextOrSelf(unsigned width) const {
  if (BitWidth < width)
    return zext(width);
  return *this;
}

APInt2 APInt2::sextOrSelf(unsigned width) const {
  if (BitWidth < width)
    return sext(width);
  return *this;
}
...
void APInt2::ashrInPlace(const APInt2 &shiftAmt) {
  ashrInPlace((unsigned)shiftAmt.getLimitedValue(BitWidth));
}

/// Arithmetic right-shift this APInt2 by shiftAmt.
/// Arithmetic right-shift function.
void APInt2::ashrSlowCase(unsigned ShiftAmt) {
...
void APInt2::lshrInPlace(const APInt2 &shiftAmt) {
  lshrInPlace((unsigned)shiftAmt.getLimitedValue(BitWidth));
}

/// Logical right-shift this APInt2 by shiftAmt.
/// Logical right-shift function.
void APInt2::lshrSlowCase(unsigned ShiftAmt) {
  tcShiftRight(U.pVal, getNumWords(), ShiftAmt);
}

/// Left-shift this APInt2 by shiftAmt.
/// Left-shift function.
APInt2 &APInt2::operator<<=(const APInt2 &shiftAmt) {
  // It's undefined behavior in C to shift by BitWidth or greater.
  *this <<= (unsigned)shiftAmt.getLimitedValue(BitWidth);
  return *this;
}

void APInt2::shlSlowCase(unsigned ShiftAmt) {
  tcShiftLeft(U.pVal, getNumWords(), ShiftAmt);
  clearUnusedBits();
}

// Calculate the rotate amount modulo the bit width.
static unsigned rotateModulo(unsigned BitWidth, const APInt2 &rotateAmt) {
  if (LLVM_UNLIKELY(BitWidth == 0))
    return 0;
  unsigned rotBitWidth = rotateAmt.getBitWidth();
  APInt2 rot = rotateAmt;
  if (rotBitWidth < BitWidth) {
    // Extend the rotate APInt2, so that the urem doesn't divide by 0.
    // e.g. APInt2(1, 32) would give APInt2(1, 0).
    rot = rotateAmt.zext(BitWidth);
  }
  rot = rot.urem(APInt2(rot.getBitWidth(), BitWidth));
  return rot.getLimitedValue(BitWidth);
}

APInt2 APInt2::rotl(const APInt2 &rotateAmt) const {
  return rotl(rotateModulo(BitWidth, rotateAmt));
}

APInt2 APInt2::rotl(unsigned rotateAmt) const {
  if (LLVM_UNLIKELY(BitWidth == 0))
    return *this;
  rotateAmt %= BitWidth;
  if (rotateAmt == 0)
...
APInt2 APInt2::rotr(const APInt2 &rotateAmt) const {
  return rotr(rotateModulo(BitWidth, rotateAmt));
}

APInt2 APInt2::rotr(unsigned rotateAmt) const {
  if (BitWidth == 0)
    return *this;
  rotateAmt %= BitWidth;
  if (rotateAmt == 0)
...
unsigned APInt2::nearestLogBase2() const {
...
APInt2 APInt2::sqrt() const {
  ...
  if (magnitude <= 5) {
    ...
    return APInt2(BitWidth, results[ (isSingleWord() ? U.VAL : U.pVal[0]) ]);
  }
  ...
  // was adapted to APInt2 from a wikipedia article on such computations.
  // See http://www.wikipedia.org/ and go to the page named
  // Calculate_an_integer_square_root.
  unsigned nbits = BitWidth, i = 4;
  APInt2 testy(BitWidth, 16);
  APInt2 x_old(BitWidth, 1);
  APInt2 x_new(BitWidth, 0);
  APInt2 two(BitWidth, 2);
  ...
  APInt2 square(x_old * x_old);
  APInt2 nextSquare((x_old + 1) * (x_old +1));
  if (this->ult(square))
    return x_old;
  assert(this->ule(nextSquare) && "Error in APInt2::sqrt computation");
  APInt2 midpoint((nextSquare - square).udiv(two));
  APInt2 offset(*this - square);
  if (offset.ult(midpoint))
    return x_old;
  return x_old + 1;
}

/// Computes the multiplicative inverse of this APInt2 for a given modulo. The
/// iterative extended Euclidean algorithm is used to solve for this value,
/// however we simplify it to speed up calculating only the inverse, and take
/// advantage of div+rem calculations. We also use some tricks to avoid copying
/// (potentially large) APInt2s around.
/// WARNING: a value of '0' may be returned,
///          signifying that no multiplicative inverse exists!
APInt2 APInt2::multiplicativeInverse(const APInt2& modulo) const {
  assert(ult(modulo) && "This APInt2 must be smaller than the modulo");
  ...
  APInt2 r[2] = { modulo, *this };
  APInt2 t[2] = { APInt2(BitWidth, 0), APInt2(BitWidth, 1) };
  APInt2 q(BitWidth, 0);
  ...
  // If this APInt2 and the modulo are not coprime, there is no multiplicative
  // inverse, so return 0. We check this by looking at the next-to-last
  // remainder, which is the gcd(*this,modulo) as calculated by the Euclidean
  // algorithm.
  if (r[i] != 1)
    return APInt2(BitWidth, 0);
...
void APInt2::divide(const WordType *LHS, unsigned lhsWords, const WordType *RHS,
...
APInt2 APInt2::udiv(const APInt2 &RHS) const {
  assert(BitWidth == RHS.BitWidth && "Bit widths must be the same");
  ...
  if (*this == RHS)
    // X / X ===> 1
    return APInt2(BitWidth, 1);
  if (lhsWords == 1) // rhsWords is 1 if lhsWords is 1.
    // All high words are zero, just use native divide
    return APInt2(BitWidth, this->U.pVal[0] / RHS.U.pVal[0]);

  // We have to compute it the hard way. Invoke the Knuth divide algorithm.
  APInt2 Quotient(BitWidth, 0); // to hold result.
  divide(U.pVal, lhsWords, RHS.U.pVal, rhsWords, Quotient.U.pVal, nullptr);
  return Quotient;
}

APInt2 APInt2::udiv(uint64_t RHS) const {
  assert(RHS != 0 && "Divide by zero?");

  // First, deal with the easy case
  if (isSingleWord())
    return APInt2(BitWidth, U.VAL / RHS);
  ...
  if (lhsWords == 1) // rhsWords is 1 if lhsWords is 1.
    // All high words are zero, just use native divide
    return APInt2(BitWidth, this->U.pVal[0] / RHS);
...
APInt2 APInt2::sdiv(const APInt2 &RHS) const {
  if (isNegative()) {
    if (RHS.isNegative())
...
APInt2 APInt2::sdiv(int64_t RHS) const {
  if (isNegative()) {
    if (RHS < 0)
      return (-(*this)).udiv(-RHS);
    return -((-(*this)).udiv(RHS));
...
APInt2 APInt2::urem(const APInt2 &RHS) const {
  assert(BitWidth == RHS.BitWidth && "Bit widths must be the same");
  if (isSingleWord()) {
  ...
  if (lhsWords == 1)
    ...
    return APInt2(BitWidth, U.pVal[0] % RHS.U.pVal[0]);

  // We have to compute it the hard way. Invoke the Knuth divide algorithm.
  APInt2 Remainder(BitWidth, 0);
  divide(U.pVal, lhsWords, RHS.U.pVal, rhsWords, nullptr, Remainder.U.pVal);
  return Remainder;
}

uint64_t APInt2::urem(uint64_t RHS) const {
...
APInt2 APInt2::srem(const APInt2 &RHS) const {
  if (isNegative()) {
    if (RHS.isNegative())
...
int64_t APInt2::srem(int64_t RHS) const {
  if (isNegative()) {
    if (RHS < 0)
      return -((-(*this)).urem(-RHS));
    return -((-(*this)).urem(RHS));
...
void APInt2::udivrem(const APInt2 &LHS, const APInt2 &RHS,
                    APInt2 &Quotient, APInt2 &Remainder) {
  ...
  if (LHS.isSingleWord()) {
    ...
    Quotient = APInt2(BitWidth, QuotVal);
    Remainder = APInt2(BitWidth, RemVal);
    return;
  }
  ...
  // Check the degenerate cases
  if (lhsWords == 0) {
    Quotient = APInt2(BitWidth, 0);    // 0 / Y ===> 0
    Remainder = APInt2(BitWidth, 0);   // 0 % Y ===> 0
    return;
  }

  if (rhsBits == 1) {
    Quotient = LHS;                   // X / 1 ===> X
    Remainder = APInt2(BitWidth, 0);   // X % 1 ===> 0
  }

  if (lhsWords < rhsWords || LHS.ult(RHS)) {
    Remainder = LHS;                  // X % Y ===> X, iff X < Y
    Quotient = APInt2(BitWidth, 0);    // X / Y ===> 0, iff X < Y
    return;
  }

  if (LHS == RHS) {
    Quotient  = APInt2(BitWidth, 1);   // X / X ===> 1
    Remainder = APInt2(BitWidth, 0);   // X % X ===> 0;
    return;
  }
...
void APInt2::udivrem(const APInt2 &LHS, uint64_t RHS, APInt2 &Quotient,
                    uint64_t &Remainder) {
  assert(RHS != 0 && "Divide by zero?");
  unsigned BitWidth = LHS.BitWidth;
...
void APInt2::sdivrem(const APInt2 &LHS, const APInt2 &RHS,
                    APInt2 &Quotient, APInt2 &Remainder) {
  if (LHS.isNegative()) {
    if (RHS.isNegative())
      APInt2::udivrem(-LHS, -RHS, Quotient, Remainder);
    else {
      APInt2::udivrem(-LHS, RHS, Quotient, Remainder);
      Quotient.negate();
    }
...
void APInt2::sdivrem(const APInt2 &LHS, int64_t RHS,
                    APInt2 &Quotient, int64_t &Remainder) {
  uint64_t R = Remainder;
  if (LHS.isNegative()) {
    if (RHS < 0)
      APInt2::udivrem(-LHS, -RHS, Quotient, R);
    else {
      APInt2::udivrem(-LHS, RHS, Quotient, R);
      Quotient.negate();
    }
...
APInt2 APInt2::usub_ov(const APInt2 &RHS, bool &Overflow) const {
  APInt2 Res = *this-RHS;
...
APInt2 APInt2::sshl_ov(const APInt2 &ShAmt, bool &Overflow) const {
...
APInt2 APInt2::ushl_ov(const APInt2 &ShAmt, bool &Overflow) const {
  Overflow = ShAmt.uge(getBitWidth());
  if (Overflow)
    return APInt2(BitWidth, 0);
...
APInt2 APInt2::sadd_sat(const APInt2 &RHS) const {
  bool Overflow;
  APInt2 Res = sadd_ov(RHS, Overflow);
  if (!Overflow)
    return Res;

  return isNegative() ? APInt2::getSignedMinValue(BitWidth)
                      : APInt2::getSignedMaxValue(BitWidth);
}

APInt2 APInt2::uadd_sat(const APInt2 &RHS) const {
  bool Overflow;
  APInt2 Res = uadd_ov(RHS, Overflow);
  if (!Overflow)
    return Res;

  return APInt2::getMaxValue(BitWidth);
}

APInt2 APInt2::ssub_sat(const APInt2 &RHS) const {
  bool Overflow;
  APInt2 Res = ssub_ov(RHS, Overflow);
  if (!Overflow)
    return Res;
...
APInt2 APInt2::usub_sat(const APInt2 &RHS) const {
  bool Overflow;
  APInt2 Res = usub_ov(RHS, Overflow);
  if (!Overflow)
    return Res;
...
APInt2 APInt2::smul_sat(const APInt2 &RHS) const {
  bool Overflow;
  APInt2 Res = smul_ov(RHS, Overflow);
  if (!Overflow)
    return Res;
  ...
  return ResIsNegative ? APInt2::getSignedMinValue(BitWidth)
                       : APInt2::getSignedMaxValue(BitWidth);
}

APInt2 APInt2::umul_sat(const APInt2 &RHS) const {
  bool Overflow;
  APInt2 Res = umul_ov(RHS, Overflow);
  if (!Overflow)
    return Res;
...
APInt2 APInt2::sshl_sat(const APInt2 &RHS) const {
  bool Overflow;
  APInt2 Res = sshl_ov(RHS, Overflow);
  if (!Overflow)
    return Res;
...
APInt2 APInt2::ushl_sat(const APInt2 &RHS) const {
  bool Overflow;
  APInt2 Res = ushl_ov(RHS, Overflow);
  if (!Overflow)
    return Res;
...
void APInt2::fromString(unsigned numbits, StringRef str, uint8_t radix) {
...
void APInt2::toString(SmallVectorImpl<char> &Str, unsigned Radix,
...
LLVM_DUMP_METHOD void APInt2::dump() const {
  SmallString<40> S, U;
  this->toStringUnsigned(U);
  this->toStringSigned(S);
  dbgs() << "APInt2(" << BitWidth << "b, "
         << U << "u " << S << "s)\n";
}
#endif

void APInt2::print(raw_ostream &OS, bool isSigned) const {
  SmallString<40> S;
...
static_assert(APInt2::APINT_BITS_PER_WORD % 2 == 0,
              "Part width must be divisible by 2!");

// Returns the integer part with the least significant BITS set.
// BITS cannot be zero.
static inline APInt2::WordType lowBitMask(unsigned bits) {
  assert(bits != 0 && bits <= APInt2::APINT_BITS_PER_WORD);
  return ~(APInt2::WordType) 0 >> (APInt2::APINT_BITS_PER_WORD - bits);
}

/// Returns the value of the lower half of PART.
static inline APInt2::WordType lowHalf(APInt2::WordType part) {
  return part & lowBitMask(APInt2::APINT_BITS_PER_WORD / 2);
}

/// Returns the value of the upper half of PART.
static inline APInt2::WordType highHalf(APInt2::WordType part) {
  return part >> (APInt2::APINT_BITS_PER_WORD / 2);
}

/// Returns the bit number of the most significant set bit of a part.
/// If the input number has no bits set -1U is returned.
static unsigned partMSB(APInt2::WordType value) {
...
static unsigned partLSB(APInt2::WordType value) {
  return findFirstSet(value, ZB_Max);
}

/// Sets the least significant part of a bignum to the input value, and zeroes
/// out higher parts.
void APInt2::tcSet(WordType *dst, WordType part, unsigned parts) {
  assert(parts > 0);
  dst[0] = part;
...
void APInt2::tcAssign(WordType *dst, const WordType *src, unsigned parts) {
  for (unsigned i = 0; i < parts; i++)
    dst[i] = src[i];
}

/// Returns true if a bignum is zero, false otherwise.
bool APInt2::tcIsZero(const WordType *src, unsigned parts) {
  for (unsigned i = 0; i < parts; i++)
    if (src[i])
      return false;
...
int APInt2::tcExtractBit(const WordType *parts, unsigned bit) {
  return (parts[whichWord(bit)] & maskBit(bit)) != 0;
}

/// Set the given bit of a bignum.
void APInt2::tcSetBit(WordType *parts, unsigned bit) {
  parts[whichWord(bit)] |= maskBit(bit);
}

/// Clears the given bit of a bignum.
void APInt2::tcClearBit(WordType *parts, unsigned bit) {
  parts[whichWord(bit)] &= ~maskBit(bit);
}

/// Returns the bit number of the least significant set bit of a number.  If the
/// input number has no bits set -1U is returned.
unsigned APInt2::tcLSB(const WordType *parts, unsigned n) {
  for (unsigned i = 0; i < n; i++) {
    if (parts[i] != 0) {
...
/// If the input number has no bits set -1U is returned.
unsigned APInt2::tcMSB(const WordType *parts, unsigned n) {
  do {
    --n;
...
/// */
void
APInt2::tcExtract(WordType *dst, unsigned dstCount, const WordType *src,
...
APInt2::WordType APInt2::tcAdd(WordType *dst, const WordType *rhs,
...
APInt2::WordType APInt2::tcAddPart(WordType *dst, WordType src,
                                 unsigned parts) {
  for (unsigned i = 0; i < parts; ++i) {
    dst[i] += src;
    if (dst[i] >= src)
...
/// DST -= RHS + C where C is zero or one.  Returns the carry flag.
APInt2::WordType APInt2::tcSubtract(WordType *dst, const WordType *rhs,
                                  WordType c, unsigned parts) {
  assert(c <= 1);
...
APInt2::WordType APInt2::tcSubtractPart(WordType *dst, WordType src,
                                      unsigned parts) {
  for (unsigned i = 0; i < parts; ++i) {
    WordType Dst = dst[i];
    dst[i] -= src;
...
void APInt2::tcNegate(WordType *dst, unsigned parts) {
...
/// return one.
int APInt2::tcMultiplyPart(WordType *dst, const WordType *src,
...
/// from both operands.
int APInt2::tcMultiply(WordType *dst, const WordType *lhs,
...
void APInt2::tcFullMultiply(WordType *dst, const WordType *lhs,
...
int APInt2::tcDivide(WordType *lhs, const WordType *rhs,
...
void APInt2::tcShiftLeft(WordType *Dst, unsigned Words, unsigned Count) {
  // Don't bother performing a no-op shift.
  if (!Count)
    return;
...
void APInt2::tcShiftRight(WordType *Dst, unsigned Words, unsigned Count) {
  // Don't bother performing a no-op shift.
  if (!Count)
    return;
...
int APInt2::tcCompare(const WordType *lhs, const WordType *rhs,
                     unsigned parts) {
  while (parts) {
    parts--;
...
APInt2 llvm::APInt2Ops::RoundingUDiv(const APInt2 &A, const APInt2 &B,
                                   APInt2::Rounding RM) {
  // Currently udivrem always rounds down.
  switch (RM) {
  case APInt2::Rounding::DOWN:
  case APInt2::Rounding::TOWARD_ZERO:
    return A.udiv(B);
  case APInt2::Rounding::UP: {
    APInt2 Quo, Rem;
    APInt2::udivrem(A, B, Quo, Rem);
    if (Rem.isZero())
      return Quo;
    return Quo + 1;
  }
  }
  llvm_unreachable("Unknown APInt2::Rounding enum");
}

APInt2 llvm::APInt2Ops::RoundingSDiv(const APInt2 &A, const APInt2 &B,
                                   APInt2::Rounding RM) {
  switch (RM) {
  case APInt2::Rounding::DOWN:
  case APInt2::Rounding::UP: {
    APInt2 Quo, Rem;
    APInt2::sdivrem(A, B, Quo, Rem);
    if (Rem.isZero())
      return Quo;
...
Optional<APInt2>
llvm::APInt2Ops::SolveQuadraticEquationWrap(APInt2 A, APInt2 B, APInt2 C,
                                           unsigned RangeWidth) {
  ...
  if (C.sextOrTrunc(RangeWidth).isZero()) {
    ...
    return APInt2(CoeffWidth, 0);
  }

  // The result of APInt2 arithmetic has the same bit width as the operands,
  ...
  // solutions.)
  APInt2 R = APInt2::getOneBitSet(CoeffWidth, RangeWidth);
  APInt2 TwoA = 2 * A;
  APInt2 SqrB = B * B;
  bool PickLow;

  auto RoundUp = [] (const APInt2 &V, const APInt2 &A) -> APInt2 {
    assert(A.isStrictlyPositive());
    APInt2 T = V.abs().urem(A);
    if (T.isZero())
      return V;
  ...
  if (B.isNonNegative()) {
    ...
    APInt2 LowkR = C - SqrB.udiv(2*TwoA); // udiv because all values > 0.
  ...
  // of SQ).
  if (PickLow)
    APInt2::sdivrem(-B - (SQ+InexactSQ), TwoA, X, Rem);
  else
    APInt2::sdivrem(-B + SQ, TwoA, X, Rem);

  // The updated coefficients should be such that the (exact) solution is
  // positive. Since APInt2 division rounds towards 0, the calculated one
...
Optional<unsigned>
llvm::APInt2Ops::GetMostSignificantDifferentBit(const APInt2 &A, const APInt2 &B) {
...
APInt2 llvm::APInt2Ops::ScaleBitMask(const APInt2 &A, unsigned NewBitWidth) {
  ...
  APInt2 NewA = APInt2::getZero(NewBitWidth);
...
void llvm::StoreIntToMemory(const APInt2 &IntVal, uint8_t *Dst,
                            unsigned StoreBytes) {
...
void llvm::LoadIntFromMemory(APInt2 &IntVal, const uint8_t *Src,
                             unsigned LoadBytes) {
