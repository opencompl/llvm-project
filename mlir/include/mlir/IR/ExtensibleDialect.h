...
public:
  ...
  using ParserFn = llvm::unique_function<ParseResult(
      DialectAsmParser &parser, std::vector<Attribute> &parsedAttributes)
                                             const>;
  ...
  get(Dialect *dialect, llvm::StringRef name, VerifierFn &&verifier);
...
public:
  ...
  static bool classof(Dialect *dialect);
